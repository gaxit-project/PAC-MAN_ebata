...
public class PinkyController : MonoBehaviour
{
    ...
    [SerializeField] private Animator PinkyAnim; //Pinkyのアニメーターを入れる
    //WarpPointの座標取得とOn/Off切り替え用
    [SerializeField] private GameObject WarpPosition_7; 
    [SerializeField] private GameObject WarpPosition_8;
    ...
    private float moveAxis_x = 10.15f; //横移動(y座標)
    ...
    void Update()
    {
        ...
        if(canMove)
        {
            ...
            if(isMovingUp && canMoveUp)
            {
                if(!canEated && !isEated)
                {
                    PinkyAnim.Play("Pinky_Up");
                }
                else if(isEated)
                {
                    PinkyAnim.Play("Eye_Up");
                }
            ...
            if(isMovingDown && canMoveDown)
            {
                if(!canEated && !isEated)
                {
                    PinkyAnim.Play("Pinky_Down");
                }
                else if(isEated)
                {
                    PinkyAnim.Play("Eye_Down");
                }
                transform.position = new Vector3(moveAxis_y, transform.position.y, 0.4f);
                transform.position += -transform.up * speed * Time.deltaTime;
            }
            if(isMovingLeft && canMoveLeft)
            {
                if(!canEated && !isEated)
                {
                    PinkyAnim.Play("Pinky_Left");
                }
                else if(isEated)
                {
                    PinkyAnim.Play("Eye_Left");
                }
            ...
            if(isMovingRight && canMoveRight)
            {
                if(!canEated && !isEated)
                {
                    PinkyAnim.Play("Pinky_Right");
                }
                else if(isEated)
                {
                    PinkyAnim.Play("Eye_Right");
                }
                transform.position = new Vector3(transform.position.x, moveAxis_x, 0.4f);
                transform.position += transform.right * speed * Time.deltaTime;
            }
    ...
    public void InitializePinkyMove() //動きを制御するbool変数の初期化
    {
        canMoveUp = false;
        canMoveDown = false;
        canMoveLeft = true;
    ...
    public void StopPinkyAnimation()
    {
        PinkyAnim.speed = 0;
    }
    ...
    {
        //ワープ関係
        if(other.CompareTag("WarpPoint_7"))
        {
            WarpPosition = WarpPosition_8;
            WarpPosition_8.SetActive(false);
            WarpPosition_7.SetActive(true);
            transform.position = WarpPosition.transform.position;
            Debug.Log("Pinky 右にワープ");
        }
        else if(other.CompareTag("WarpPoint_8"))
        {
            WarpPosition = WarpPosition_7;
            WarpPosition_7.SetActive(false);
            WarpPosition_8.SetActive(true);
            transform.position = WarpPosition.transform.position;
            Debug.Log("Pinky 左にワープ");
        }
        else if(other.CompareTag("Recover"))
        {
            WarpPosition_7.SetActive(true);
            WarpPosition_8.SetActive(true);
            Debug.Log("Pinky用のワープポイントが回復");
        }
        //目標位置の更新
        ...
        {
            if(PlayerController.currentPlayerPosition.y >= transform.position.y && !isEated)
            {
                targetPosition = PlayerController.currentPlayerPosition + transform.up;
            }
            else if(!isEated)
            {
                targetPosition = PlayerController.currentPlayerPosition - transform.up;
        ...
        //ゴーストの移動関係+追跡アルゴリズム(目標位置に回り込むように移動する)
